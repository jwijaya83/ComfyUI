...
- **Fetched at boot** from HuggingFace via RunPod **Model Caching** (or a network volume),
  listed in `models_manifest.json`, symlinked by `fetch_models.py` (~40 GB total): the
  int8-convrot diffusion model, the Gemma text encoder, the video + audio VAEs, and the
  persona LoRAs.
- **LTX 2.5.** Both per-turn workflows load `UNETLoader(diffusion_models/
  ltx-2.5-22b-distilled-transformer-comfy-int8-convrot)` plus two `VAELoaderKJ`
  (`vae/ltx-2.5-{video,audio}-vae-*-bf16`) and a single `CLIPLoader`
  (`text_encoders/gemma4-12b-with-proj-ltx-2.5-comfy-int8-convrot`) — unlike LTX 2.3, the
  text-projection head ships baked into that one file, so there's no `DualCLIPLoader` /
  separate projection fetch anymore. All of it is mirrored into the single
  `jwijaya17/aichat` repo so the endpoint still needs only **one** Model-Caching repo.
...
like `ltx-2.5-22b-distilled-transformer-comfy-int8-convrot.safetensors` is found ONLY
...
**flat**, so e.g. `filename: "ltx-2.5-video-vae-conv-bf16.safetensors"` →
`target: "vae/ltx-2.5-video-vae-conv-bf16.safetensors"`. The `.dockerignore`
...
- **Per-turn default LoRA.** `basic_workflow` / `latent_injection` now default node 4990 to
  `yunaKoharu_yunaxkoharuz_young_woman.safetensors`, which **is** in the manifest, so a
